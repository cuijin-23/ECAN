...
class ConnDataset(Dataset):
    ...
    def pad_data(self, batch_data):
        ...
        batch_category_label_id = []
        batch_sentiment_label_ids = []

        if len(batch_data) == 0:
            return
        for item in batch_data:
            sen_index, catagory, is_masked, text_indices, input_ids, attention_mask, token_starts, token_start_mask, dependency_graph, polarity, category_label_id,sentiment_label_ids = item['sen_index'], item['catagory_index'], item['is_masked'], item['text_indices'], item['input_ids'],\
                                                                                                                                                 item['attention_mask'],item['token_starts'], item['token_start_mask'], item['dependency_graph'], \
                                                                                                                                                 item['polarity'], item['category_label_id'], item['sentiment_label_ids']
            # xlnet for sa
            ...
            # batch_cat_polarity.append(cat_polarity)
            batch_dependency_graph.append(dependency_graph)
            batch_category_label_id.append(category_label_id)
            batch_sentiment_label_ids.append(sentiment_label_ids)
        ...
        return { \
            ...
            'is_masked': torch.tensor(batch_is_masked),
            ...
            # 'cat_polarity': torch.tensor(batch_cat_polarity),
            'polarity': torch.tensor(batch_polarity),
            'category_label_id':torch.tensor(batch_category_label_id, dtype=torch.long),
            'sentiment_label_ids':torch.tensor(batch_sentiment_label_ids, dtype=torch.long),
        }
    ...
    def prepareData(self, idx):
        ...
        for i in range(len(pos_sen_list)):
            ...
            original_line = text + ' <sep>  ' + '<cls>'
            # dependency_adj

            prompt = text + ' <sep> ' + ' The category is : <mask> ? ' + ' <sep> ' + '<cls>'
            ...
            # inputs = self.tokenizer.encode_plus(prompt, add_special_tokens=True, return_tensors='pt')
            input_ids = encodings['input_ids'][0]

            is_masked = [int(i == self.tokenizer.mask_token_id) for i in encodings['input_ids'][0]]

            input_ids = pad_and_truncate(input_ids, max_length)
            input_ids = input_ids.astype(dtype='int64')
            is_masked = pad_and_truncate(is_masked,max_length)
            is_masked = is_masked.astype(dtype='int64')
            ...
            catagory = pos_sen_list[i][1]
            # print(catagory)
            for j in range(0, len(catagory)-1, 2):
                # text_left, _, text_right = [s.lower().strip() for s in pos_sen_list[i][j + 1].partition("$T$")]
                # aspect = pos_sen_list[i][j + 2]
                # entity = catagory.replace(" ", "#").upper()
                entity = catagory[j].split("#")[0]#.lower()
                ...
                polarity_in = catagory[j+1].copy()
                polarity_in = polarity_in.pop()
                polarity = int(polarity_in) + 1
                category_label_id[label_map_category[entity]] = 1
                category_relation_list.append(label_map_category[entity])
                sentiment_label_id[label_map_category[entity]] = label_map_sentiment[str(polarity_in)]
                sentiment_label_ids = [label_map[sentiment_label_id[i]] for i in range(len(sentiment_label_id))]

            sent_data = {
                'sen_index': i,
                'catagory_index': entity,
                'is_masked': is_masked,
        ...
        encodings = self.tokenizer.encode_plus(pos_span, add_special_tokens=False,
                                               return_tensors='pt', return_attention_mask=True, pad_to_max_length=True)
