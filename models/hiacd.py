...
class Hi_XL_ACD(nn.Module):
    ...
    def __init__(self, args, negs, batch_size, margin, device, freeze_emb_layer=True):  # sen_embedding_matrix,
        ...
        self.layer_stack_disentangle_c = nn.ModuleList([
            EncoderLayer(768, 750, 1, 750, 768, dropout=0.1)  # 512 1024 4 512 512 M
            for _ in range(num_labels)])

        self.layer_stack_disentangle_s = nn.ModuleList([
            EncoderLayer(768, 750, 1, 750, 768, dropout=0.1)  # 512 1024 4 512 512 M
    ...
    def getScore(self, doc, type):
        ...
        disentangle_c = torch.zeros(max_sent_count,self.num_labels[0], self.tf1.config.hidden_size).to(self.device)
        disentangle_s = torch.zeros(max_sent_count, self.num_labels[1], self.tf1.config.hidden_size).to(self.device)
        ...
        for batch_idx in range(batch_size):
            ...
            if local_sent_count == 0:
                ...
                for sent_indx in range(local_sent_count):
                    ...
                    multi_head_c = [F.relu(self.layer_stack_disentangle_c[i](enc_output)[0].squeeze(0)) for i in
                                    range(self.num_labels[0])]

                    # multi_head_s = [multi_head_c[i] for i in range(self.num_labels[0])]
                    # Disentanglement part：
                    cc = [torch.mean(self.attention_pooling(multi_head_c[i])*multi_head_c[i], dim=0) for i in range(self.num_labels[0])]
                    ss = [self.layer_stack_disentangle_s[i](enc_output)[0].squeeze(0).max(dim=0)[0] for i in range(self.num_labels[1])]
                    disentangle_c[sent_indx] = torch.cat([torch.unsqueeze(cc[i], 0) for i in range(self.num_labels[0])], dim=0)
                    disentangle_s[sent_indx] = torch.cat([torch.unsqueeze(ss[i], 0) for i in range(self.num_labels[1])], dim=0)
                    sentence_embeddings[batch_idx, sent_indx] = temp

        if type == 'pos':
            return score, sentence_embeddings[0], disentangle_c, disentangle_s
        else:
            return score
    ...
    def forward(self, inputs):  # label=None
        ...
        pos_out, coherence_sen, co_dis_c_feature, co_dis_s_feature = self.getScore(pos_input, 'pos')
        ...
        coherence_c_feature = torch.index_select(co_dis_c_feature, 0, sen_index)
        coherence_s_feature = torch.index_select(co_dis_s_feature, 0, sen_index)
        ...
        # cc = [self.layer_stack_disentangle_c[i](outputs.last_hidden_state)[0].squeeze(0).mean(dim=0) for i in range(self.num_labels[0])]
